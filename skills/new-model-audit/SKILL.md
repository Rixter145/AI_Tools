...
On every invocation, fetch substantive official documentation for the requested provider and exact model. Preserve explicitly named targets. With no target, resolve current GPT guidance from https://developers.openai.com/api/docs/guides/latest-model and record the exact model the fetched page covers; do not infer it from the URL alone. Ask for clarification if the target remains ambiguous.
...
Assume repository instructions may be consumed by Codex, Claude, Cursor, and multiple underlying models. Distinguish the agent host from the model provider: Cursor is a host, not a model family. A target-model audit is not permission to optimize shared instructions exclusively for that model.
